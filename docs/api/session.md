...
:::{note}
`TradingSession` は `enum.Enum`（`StrEnum` ではない）です。
文字列値の取得には `.value` プロパティを使用してください（例: `session.value == "day"`）。
`session == "day"` のような文字列との直接比較は動作しません。
:::
...
| セッション | 開始 | 終了 | 境界 | 備考 |
|-----------|------|------|------|------|
| NIGHT（前日分） | 00:00 | 06:00 | 06:00 を含まない | 前取引日のナイトセッション継続 |
| CLOSED（ギャップ） | 06:00 | 08:45 | 06:00 を含む、08:45 を含まない | プレマーケット |
| DAY | 08:45 | 15:45 | 両端含む | 日中立会 |
| CLOSED（ギャップ） | 15:45超 | 17:00 | 15:45 を含まない、17:00 を含まない | セッション間 |
| NIGHT | 17:00 | 翌06:00 | 17:00 を含む、06:00 を含まない | ナイトセッション |
