...
7. **Given** JPXデリバティブ市場とJPX公式休業日データに含まれる日, **When** `market.is_business_day(休業日)` を実行, **Then** `False` が返される
...
- 年末年始境界で正しく判定される（JPX公式Excelデータに基づく、ハードコード禁止）
...
**注**: 休業日・祝日データはすべてJPX公式Excelデータに従う（ハードコード・計算ロジック禁止）。
- 休業日データ: https://www.jpx.co.jp/derivatives/rules/holidaytrading/nlsgeu000006hweb-att/nlsgeu000006jgee.xlsx
