...
2. **独立性**: 実行時の外部API依存なし（スタンドアロン動作）
3. **一次情報の参照**: SQ日・休日判定は計算ではなく公式データを参照（データは利用者が用意）
...
トレーダーやシステムが、指定した市場・時刻がどの取引セッションに属するかを知りたい。また、取引可能時間かどうかを判定したい。
...
- データファイルが未設定または見つからない場合、適切なエラーを返す
...
- **FR-025**: システムは市場（Market）を抽象化したインターフェースを提供しなければならない
- **FR-026**: 各市場は固有のタイムゾーン、営業日ルール、取引セッション定義を持たなければならない
- **FR-027**: システムは利用可能な市場の一覧を取得できなければならない
- **FR-028**: システムは市場識別子により特定の市場を取得できなければならない
- **FR-029**: 市場がサポートしない機能（例: 株式市場のSQ日）を呼び出した場合、適切なエラーを返さなければならない

#### 限月正規化

- **FR-001**: システムは日本語表記の限月（「26年3月限」「2026年3月限」形式）を解析できなければならない
- **FR-002**: システムは正規化された限月をYYYYMM形式、YYMM形式、日本語形式に変換できなければならない
- **FR-003**: システムは2桁年号を2000〜2099年の範囲で解釈しなければならない
- **FR-004**: システムは無効な限月形式に対してエラーを返さなければならない

#### 営業日判定

- **FR-005**: システムは指定された市場の公式データに基づいて営業日判定を行わなければならない
- **FR-006**: システムは市場の休場日ルール（週末、祝日等）に従って判定しなければならない
- **FR-007**: JPXデリバティブ市場では12月31日と1月2日を休場日として判定しなければならない
- **FR-008**: システムは指定市場における指定日の前営業日・翌営業日を取得できなければならない
- **FR-009**: システムは指定市場における指定期間内の営業日リストを取得できなければならない
- **FR-010**: システムは指定市場における指定期間内の営業日数をカウントできなければならない

#### SQ日参照

- **FR-011**: システムは指定された市場の公式データに基づいてSQ日を参照しなければならない
- **FR-012**: システムは指定市場・年月の月次SQ日を取得できなければならない
- **FR-013**: システムは指定市場において指定日がSQ日かどうかを判定できなければならない
- **FR-014**: システムは指定市場の年間または月間のSQ日一覧を取得できなければならない
- **FR-015**: システムはデータが存在しない年月に対して適切なエラーを返さなければならない

#### 取引セッション管理

- **FR-016**: システムは指定された市場の取引セッション定義に基づいてセッションを識別しなければならない
- **FR-017**: システムは指定市場において指定時刻が属するセッションを返せなければならない
- **FR-018**: システムは指定市場において指定時刻が取引可能時間かどうかを判定できなければならない
- **FR-019**: JPXデリバティブ市場ではギャップ期間（15:46〜16:59, 06:01〜08:44）を取引時間外として判定しなければならない
- **FR-020**: システムはタイムゾーン情報のない時刻を拒否しなければならない

#### タイムゾーン変換

- **FR-023**: システムはUTCで与えられた時刻を市場固有のタイムゾーンに換算して処理しなければならない
- **FR-024**: システムは任意のタイムゾーンで与えられた時刻を内部で市場固有のタイムゾーンに変換して判定しなければならない

#### データ管理

- **FR-021**: システムは市場ごとに利用者が用意したデータファイルのパスを設定できなければならない（データは同梱しない）
- **FR-022**: システムはデータファイルが見つからない場合に適切なエラーを返さなければならない
...
- **SC-007**: データファイル未設定時に、ユーザーが対処方法を理解できるエラーメッセージを返す
...
- 各市場の公式データのフォーマットは年度間で大きく変更されない
- 利用者は各市場の公式サイトからデータを取得し、システムに設定する責任を負う
